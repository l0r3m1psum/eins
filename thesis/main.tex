...
introduced by Einstein in 1916 in \cite{...}
...
\begin{center}
	% NOTE(Diego): questa sezione si capisce più di prima, ma questo grafico non non va bene. Innanzitutto se non ne parli nel testo dicendo "figura qua sotto" o simile lo devi mettere nell'ambiente figura con una didascalia adeguata e farci un riferimento con \ref{} dal testo. Poi è palesemente generato e non in grafica vettoriale che non va bene. Come regola generale non parlare di cose che non conosci. Per produrre un diagramma di qualità con un LLM digli di usare MetaPost o Tikz o Mermaid o altro. Se non ti senti di fare una cosa di qualità mettere una sezione tanto per non serve a nulla. Se vuoi portare esempi significativi usa roba dal mondo delle reti neurali.
...
% NOTE(Diego): l'esempio di prima ci poteva anche stare ma qua comincia a diventare una cosa molto più complessa e parlare di Navier-Stoken senza introdurre l'equazione, il calcolo differenziale con gli indici e mostrare che la contrazione finale è una moltiplicazione vettore matrice non ha molto senso.
...
% NOTE(Diego): la frase "no single algorithm" è riservata solo per i caso di non calcolabilità. Io capisco che tu vuoi intendere "nessun singolo metodo" perché se parli di algoritmi tu puoi semplicemente prendere i vari algoritmi più semplici che hai creato e unirli in uno singlo che in base ad una condizione decide quale usare.
...
% NOTE(Diego): source anonimity non mi è chiaro.
...
% NOTE(Diego): questa frase qua sotto è un po' bombastica.
By baking the geometry of the tensors directly into the instruction stream, we eliminate the CPU cycles typically wasted on managing loop metadata and index validation.
...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|}
		...
		% NOTE(Diego): allora capisco perché vuoi fare questa tabella come ma "Computational Efficiency & Extreme (hardware-level optimization)" sembra una cosa scritta da un amatoriale. Per queste cose è meglio far parlare i numeri degli esperimenti. Inoltre suona troppo troppo ChatGPT sempre per il linguaggio bombastico che deve far sembrare che qualunque cosa fai stai tirando dei missili sulla luna. Ti consiglio di riscrivere tutte le parti con un linguaggio più moderato. Questo vale anche per le righe sotto.
...
% NOTE(Diego): mi pare che rank non sia definito da nessuna parte nel testo.
% NOTE(Diego): questa dimostrazione non si capisce.
...
% NOTE(Diego) fare due micro sotttosesioni per metterci una tabella e poi un paragrafo non ha senso.
